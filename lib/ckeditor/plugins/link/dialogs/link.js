...
CKEDITOR.dialog.add( 'link', function( editor )
{
	...
	// Load JSON-data:
	var
		SYMPHONY = window.location.toString().match(/^(.+\/symphony)/)[1],
		symPages = [];
	jQuery.ajax({
		url: SYMPHONY + '/extension/ckeditor/pages/',
		async: false,
		dataType: 'json',
		success: function(data)
		{
			jQuery(data).each(function(){
				symPages.push([this.title, this.url]);
			});
		}
	});		
								...
								[
									{
										id : 'sym-page',
										type : 'select',										
										label : 'Or select a Symphony Page:',
										items : symPages,
										onChange : function(data)
										{
											var my_id = this.getDialog().getContentElement('info', 'urlOptions').domId;
											// Use some jQuery to perform the magic (since we're in Symphony anyway:)
											jQuery("input[type=text]", document.getElementById(my_id)).val(data.data.value);
										}
									},
								],
								setup : function( data )
